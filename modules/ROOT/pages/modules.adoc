...
** К данному типу относятся модули: _xref:platform::index.adoc[{pl}]_, _xref:workerservice::index.adoc[Служба {ws}]_, _xref:backoffice::index.adoc[{bo}]_, _xref:documentmgmt::index.adoc[{dm}]_, _xref:approval::index.adoc[{ad}]_, _xref:workflow::index.adoc[{wf}]_, _xref:winclient::index.adoc[{wincl}]_, _xref:webclient::index.adoc[{wc}]_ и _{to}_.
